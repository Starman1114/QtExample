...
signed char OpenCOMDevice(const unsigned long ulPortNo,const unsigned long ulBaundrate)
{
    ...
    QString portname =QString("%1").arg(PortName);
    if((hComDev[ulPortNo] = CreateFile(portname.toStdWString().c_str(),GENERIC_READ|GENERIC_WRITE,0,NULL,OPEN_EXISTING,FILE_FLAG_OVERLAPPED ,NULL))==INVALID_HANDLE_VALUE)
	{
		dwRes=GetLastError();
		return -1;
	}
