    ...
    ui(new Ui::Widget)
{
    ...
    connect(ui->filterEdit, SIGNAL(returnPressed()),this, SLOT(showFiles()));
...
bool Widget::filterOK(QString file_suffix)
{
    QStringList strings=ui->filterEdit->text().split(QRegExp("[/]"));
    for(int k=0;k<strings.length();k++){
        if(file_suffix==strings[k])
            return true;
    }
