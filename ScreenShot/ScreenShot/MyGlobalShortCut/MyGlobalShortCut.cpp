...
bool  MyGlobalShortCut::registerHotKey()
{
    ...
    return RegisterHotKey(nullptr, nativeMods ^ nativeKey, nativeMods, nativeKey);
}
bool  MyGlobalShortCut::unregisterHotKey()
{
    return UnregisterHotKey(nullptr, (quint32)nativeModifiers(mods) ^ (quint32)nativeKeycode(key));
}
